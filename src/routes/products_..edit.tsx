...
import { uploadImages, createProduct } from "@/lib/supabaseApi";
import { formatVND } from "@/lib/format";
import type { Product, WarehouseType } from "@/lib/mockData";

export const Route = createFileRoute("/products_/edit")({
  head: () => ({ meta: [{ title: "Tạo sản phẩm — Xus Admin" }] }),
...
function ProductNewPage() {
  ...
  const setExpiryPreset = (days: 30 | 60) => {
    const expiry = calcExpiry(form.productionDate, days);
    setForm((f) => ({ ...f, expiryDate: expiry }));
  };

  const handleProductionDate = (val: string) => {
    setForm((f) => ({
      ...f,
      productionDate: val,
      // Auto-recalculate expiry if it was already set
  ...
  const submit = async () => {
    ...
    try {
      ...
      const p: Product = { id: `p-${Date.now()}`, ...form, images: finalImages };
      addProduct(p);
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Khởi tạo sản phẩm và cấu hình tồn kho ban đầu"
        actions={
          <>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => handleProductionDate(e.target.value)}
                    className="mt-1.5"
                  />
                </div>
                <div>
                  ...
                  <div className="flex gap-2 mt-1.5">
                    <button
                      ...
                      className="text-xs px-2 py-0.5 rounded bg-muted hover:bg-primary hover:text-primary-foreground transition"
                    >
                      +30 ngày
                    </button>
                    <button
